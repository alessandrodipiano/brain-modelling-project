#IMPLEMENTATION OF A SINGLE TRIAL FROM THE MONKEY EXPERIMENT
#two input sequences are created and fed to two separate neuron populations
#activity of those populations is plotted together with the strength of stimulus to each
#higher activity is interpreted as the "choice" of one of the two inputs

#TO DO: 
#  - a causal exponential filter (defined in the paper) needs to be applied 
#     to the recorded activity
#  - from each run, smoothed data about the activity, the network "choice", 
#     and the strength of evidence of each input needs to be stored
#  - some way to manage multiple runs and aggregate their results needs to be implemented

from brian2 import *
import numpy as np

#-----------------------------------------------------------------------------------------------
# Generate the stimulus input
# (we assume the 'choose high' setting)
# -----------------------------------------------------------------------------------------------
mu0 = 30.0 * Hz  # stimulus input mean when bar height is medium
mu1 = 1.0 * Hz  # multiplier for stimulus intensity (bar height)
sigN = 12 # SD of the low-variability sequence
sigB = 24 # SD of the high-variability sequence

Z = np.random.uniform(-0.25, 0.25, 8)
muN = 50 + Z*sigN
muB = 50 + Z*sigB 

hN = np.clip(np.random.normal(muN, sigN), 1, 99)
hB = np.clip(np.random.normal(muB, sigB), 1, 99)

muPoisN = mu0 + mu1*(hN - 50)
muPoisB = mu0 + mu1*(hB - 50)
print(hN, hB)
print("mean: ", np.mean(hN), np.mean(hB))
print(muPoisN, muPoisB)
# -----------------------------------------------------------------------------------------------
...
mu0 = 30.0 * Hz  # stimulus input mean when bar height is medium
mu1 = 1.0 * Hz  # multiplier for stimulus intensity (bar height)
stim_interval = 250.0 * ms  # stimulus changes every 50 ms

stim_arrayN = TimedArray(muPoisN, dt=stim_interval)
stim_arrayB = TimedArray(muPoisB, dt=stim_interval)

stim_on = 1000 * ms # stimulus onset
stim_off = 3000 * ms  # stimulus offset, stim_on + 8*stim_interval
runtime = 5000 * ms # total simulation time
...
NE = 1600  # number of excitatory neurons (1600)
NI = 400  # number of inhibitory neurons (400)
subN = 240 # number of neurons in decision pools
...
stiminputE1 = PoissonGroup(subN, rates='int(t > stim_on and t < stim_off) * stim_arrayN(t-1000*ms)', name='stiminputE1')
stiminputE2 = PoissonGroup(subN, rates='int(t > stim_on and t < stim_off) * stim_arrayB(t-1000*ms)', name='stiminputE2')
...
# -----------------------------------------------------------------------------------------------
# Define the smoothing filter (causal exponential) 
# -----------------------------------------------------------------------------------------------
tau_filter = 20*ms
ts = 0.001*ms
...
# Run the simulation
run(runtime, report='stdout', profile=False)
